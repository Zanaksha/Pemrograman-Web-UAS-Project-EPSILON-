...
<div class="main">
    <div class="top-bar">
        <h5 class="mb-0 fw-bold">Manage Cars</h5>
        <a href="/admin/cars/create" class="btn btn-primary btn-sm">+ Add Car</a>
    </div>
            ...
            <table class="table table-hover mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @if($car->image)
                                @if(str_starts_with($car->image, 'http'))
                                    <img src="{{ $car->image }}" width="60" height="40" style="object-fit:contain;">
                                @else
                                    <img src="{{ asset('images/' . basename($car->image)) }}" width="60" height="40" style="object-fit:contain;">
                                @endif
                            @endif
                        </td>
                        <td>{{ $car->name }}</td>
                        ...
                        <td>{{ $car->price ? 'Rp ' . number_format($car->price, 0, ',', '.') : '-' }}</td>
                        <td>
                            <a href="/admin/cars/{{ $car->id }}/edit" class="btn btn-warning btn-sm">Edit</a>
                            <form method="POST" action="/admin/cars/{{ $car->id }}" style="display:inline;">
                                @csrf
                                @method('DELETE')
