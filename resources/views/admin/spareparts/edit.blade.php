...
<div class="sidebar">
    ...
    <a href="/admin/spareparts"  class="active"><i class="bi bi-wrench me-2"></i> SpareParts</a>
...
<div class="main">
    <div class="top-bar">
        <h5 class="mb-0 fw-bold">Edit Sparepart — {{ $sparepart->name }}</h5>
    </div>
    ...
    <div class="card border-0 shadow-sm p-4" style="max-width:700px;">
        <form method="POST" action="{{ route('admin.spareparts.update', $sparepart->id) }}">
            @csrf
            @method('PUT')
            <div class="row g-3">
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="part_number" class="form-control" value="{{ $sparepart->part_number }}">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Category</label>
                    <select name="category" class="form-select">
                        @foreach(['Engine','Brake','Filter','Electrical','Suspension','Body'] as $cat)
                        <option {{ $sparepart->category == $cat ? 'selected' : '' }}>{{ $cat }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-6">
                    <label class="form-label">Compatible Model</label>
                    <input type="text" name="compatible_model" class="form-control" value="{{ $sparepart->compatible_model }}">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Price</label>
                    <input type="number" name="price" class="form-control" value="{{ $sparepart->price }}" required>
                </div>
                <div class="col-md-6">
                ...
                <div class="col-12">
                    ...
                    <textarea name="description" class="form-control" rows="3">{{ $sparepart->description }}</textarea>
                </div>
                <div class="col-12">
                    <label class="form-label">Image URL</label>
                    <input type="text" name="image" class="form-control" value="{{ $sparepart->image }}">
                </div>
                <div class="col-12 mt-3">
                    <button type="submit" class="btn btn-primary px-4">Update Sparepart</button>
